...
echo "	--> prepare for later use of sonarqube for application analysis"
SONARQUBE_VERSION=7.0
[ -f resources/openshift-sonarqube-embedded-template.yaml ] || curl -sS -L http://bit.ly/openshift-sonarqube-embedded-template > resources/openshift-sonarqube-embedded-template.yaml
oc new-app -f resources/openshift-sonarqube-embedded-template.yaml --param=SONARQUBE_VERSION=${SONARQUBE_VERSION} --param=SONAR_MAX_MEMORY=4Gi
oc tag sonarqube:${SONARQUBE_VERSION} sonarqube:latest

oc new-build --name=sonarqube-ext --context-dir=resources/sonarqube https://github.com/michaelepley/openshift-demo-nauticalcharts.git

# TODO: modify the previously created deploymentconfig to deploy the sonarqube-ext
